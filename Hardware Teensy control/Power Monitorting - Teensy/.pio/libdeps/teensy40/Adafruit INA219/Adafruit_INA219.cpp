...
/*!
 *  @brief  Configures to INA219 to be able to measure up to 32V and
 *          desired current.
 *  @note   These calculations based on the input of Rshunt and Imax
 */
void Adafruit_INA219::setCalibration(float Rshunt) {
  // By default we use a pretty huge range for the input voltage,
  // which probably isn't the most appropriate choice for system
  // that don't use a lot of power.  But all of the calculations
  // are shown below if you want to change the settings.  You will
  ...
  // VBUS_MAX = 32V             (Assumes 32V, can also be set to 16V)
  // VSHUNT_MAX = 0.32          (Assumes Gain 8, 320mV, can also be 0.16, 0.08,
  // 0.04) RSHUNT = Rshunt               (Resistor value in ohms)
  ...
  // 3. Calculate possible range of LSBs (Min = 15-bit, Max = 12-bit)
  // MinimumLSB = MaxExpected_I/32767
  // MinimumLSB = 0.0009765923             (1mA per bit)
  // MaximumLSB = MaxExpected_I/4096
  // MaximumLSB = 0.0078125             (8mA per bit)
  ...
  // 5. Compute the calibration register
    float Cal = trunc (0.04096 / (0.001 * Rshunt));
  // Cal = 4096 (0x1000)
  ...
  // 8. Compute the Maximum Power
  // MaximumPower = Max_Current_Before_Overflow * VBUS_MAX
  // MaximumPower = 32.767A * 32V
  // MaximumPower = 1048.544W

  // Set multipliers to convert raw current/power values
  ina219_currentDivider_mA = 1; // Current LSB = 1mA per bit (1000/1000 = 1)
  ina219_powerMultiplier_mW = 20; // Power LSB = 20mW per bit
